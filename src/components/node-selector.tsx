...
export function NodeSelector({children, open, onOpenChange}: NodeSelectorProps) {
  ...
  const handleNodeSelect = useCallback((selection: NodeTypeOptions) => {
    if (selection.type === NodeType.MANUAL_TRIGGER) {
      ...
      if (hasManualTrigger) {
        toast.error("Only one manual trigger per workflow allowed")
        return
      }
    }

    setNodes((nodes) => {
      const hasInitialTrigger = nodes.some((node) =>
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        <SheetContent side={"right"} className={"w-full sm:max-w-md overflow-y-auto"}>
          ...
          <div>{triggerNodes.map((nodeType) => {
                ...
                return <div
                    className={"w-full justify-start h-auto py-5 px-4 rounded-none cursor-pointer border-l-2 border-transparent hover:border-l-primary hover:bg-muted-foreground/10"}
                    key={nodeType.type}
